...
#debut element fenêtres secondaires contenant le canvas et les boutons 
            ...
            self.bouton_sauvegarder =tk.Button(master = self.fenetre_etiquettes, text = "sauvegarder les coordonnées", command =self.coord_fichier)
...
#fonction permettant au bouton "charger" d'importer l'image dans la listbox
      ...
      def coord_fichier(self) :
      #prend les coordonnées du rectangle et les inscrit dans un fichier
        fichier_json = FD.asksaveasfilename()
        try:
         data = self.position[0], self.position[1],self.position[2], self.position[3]
        except:
         data = self.position[1], self.position[2],self.position[3], self.position[4]
        
        with open (fichier_json, 'w', encoding='utf-8', newline = '\n' ) as fd : 
             r=fd.write()
        json.dump(s, r)
